...
function getWidgetProperties(
  ...
  const propNodes = [
    ...propsInterface.getProperties(),
    ...propsInterface.getMethods()
  ];
...
export default function(config: { [index: string]: string }) {
  ...
  return Object.keys(config).reduce((props, widgetName): {
    ...
    const propsInterface = sourceFile.getInterface(
      getPropertyInterfaceName(widgetName)
    );
    if (!propsInterface) {
      console.warn(
        `could not find interface for ${widgetName} ${getPropertyInterfaceName(
          widgetName
        )}`
      );
      return props;
